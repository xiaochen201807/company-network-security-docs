---
aliases:
  - "PSIRT 与漏洞披露"
type: "standard"
domain: "psirt"
phase:
  - "respond"
priority: "P2"
status: "active"
tags:
  - "security"
  - "security/type/standard"
  - "security/domain/psirt"
  - "security/priority/p2"
  - "security/phase/respond"
parent:
  - "[[docs/公司安全知识图谱]]"
related:
  - "[[docs/09-漏洞管理与授权渗透测试]]"
  - "[[docs/18-产品安全与默认安全]]"
  - "[[docs/11-安全事件与应急响应]]"
  - "[[docs/27-第三方风险管理]]"
---
# 20 - PSIRT 与漏洞披露

> 本文档用于处理外部安全研究人员、客户、合作伙伴或第三方发现的产品漏洞，建立从接收、确认、修复、协调披露到客户通知的 Product Security Incident Response Team（PSIRT）流程。

## 1. 目标

- 外部漏洞有明确报告渠道。
- 安全报告不会丢失在普通客服流程中。
- 对研究人员进行专业、及时沟通。
- 修复、版本、Advisory 和客户通知统一协调。
- 披露过程兼顾客户安全与透明度。

## 2. 适用范围

- 公司产品。
- SaaS。
- API。
- 客户私有化产品。
- SDK。
- 移动 App。
- 公司官方互联网资产。

## 3. PSIRT 角色

| 角色 | 职责 |
|---|---|
| PSIRT Lead | 总协调 |
| Product Security | 技术验证与风险 |
| Product Team | 修复 |
| Release | 版本发布 |
| Legal | 法律/披露支持 |
| Communications | 客户/公众沟通 |
| Customer Support | 客户支持 |

## 4. 安全报告入口

至少提供一个稳定渠道：

- security@company。
- Security Page。
- VDP Form。

并定期验证邮箱/表单可用。

## 5. security.txt

成熟阶段可考虑在互联网资产部署标准化 security.txt，方便研究人员找到安全联系方式。

## 6. 报告接收字段

- Reporter。
- Contact。
- Asset/Product。
- Version。
- Vulnerability。
- Reproduction。
- Impact。
- Evidence。
- Disclosure Intent。

## 7. 自动回复

报告收到后 SHOULD 快速确认：

- 已收到。
- Case ID。
- 后续沟通方式。
- 不承诺未评估的奖励/结果。

## 8. Case ID

示例：

~~~text
PSIRT-2026-000001
~~~

## 9. 初筛

判断：

- 是否属于公司资产。
- 是否重复。
- 是否真实。
- 是否已有修复。
- 是否需要紧急升级。
- 是否包含危险数据。

## 10. 技术验证

验证过程必须遵循 09 漏洞管理规则，并尽量在安全测试环境复现。

## 11. 风险定级

结合：

- CVSS。
- 产品影响。
- 默认配置。
- 远程利用。
- 认证要求。
- 客户暴露。
- 数据影响。
- Exploit。

## 12. Critical 报告

Critical 触发：

- 产品团队紧急通知。
- 发布负责人。
- PSIRT Lead。
- 必要时公司 Incident Response。

## 13. Duplicate

重复报告应礼貌说明已经跟踪，但不向无权限人员披露其他研究员身份或未公开细节。

## 14. Out of Scope

如果不属于公司资产：

- 明确告知。
- 不鼓励继续超范围测试。
- 如能确认正确联系人，可提供适当转介。

## 15. Safe Harbor

如果公司建立公开 VDP，应明确在研究人员遵守规则情况下，公司对善意安全研究的态度。具体法律文本必须由法务审核。

## 16. VDP Scope

公开说明：

- In Scope。
- Out of Scope。
- 禁止 DDoS。
- 禁止社会工程（如适用）。
- 数据最小化。
- 不访问他人数据。
- 停止条件。

## 17. Bug Bounty

若未来建立奖励计划，需要额外定义：

- Scope。
- Reward Tier。
- Eligibility。
- Duplicate。
- Quality Requirement。
- Payment。
- Disclosure。

## 18. Researcher Communication

沟通原则：

- 尊重。
- 及时。
- 不失联。
- 不要求不合理长期保密。
- 对修复时间保持透明。

## 19. 修复协调

~~~text
Report
 ↓
Validate
 ↓
Risk
 ↓
Affected Versions
 ↓
Fix
 ↓
Test
 ↓
Release
 ↓
Advisory
 ↓
Disclosure
~~~

## 20. 多版本影响

明确：

- 当前版本。
- LTS。
- 旧版本。
- EOL。

哪些版本提供修复由 Support Policy 决定。

## 21. Embargo

修复期间可与 Reporter 协调合理 Embargo，以降低客户在补丁可用前被攻击的风险。

## 22. CVE Coordination

根据公司产品和漏洞情况决定是否：

- 申请 CVE。
- 与 CNA 协调。
- 引用上游 CVE。

CVE 申请与否不影响内部必须修复。

## 23. Upstream Vulnerability

如果漏洞来自开源上游：

- 确认上游状态。
- 不提前不负责任公开未修复漏洞。
- 评估公司是否需要临时 Patch。
- 跟踪上游版本。

## 24. Downstream Notification

如果公司组件被下游产品使用，应识别受影响产品并协调修复。

## 25. Advisory

正式安全公告包含：

- Identifier。
- Summary。
- Affected Version。
- Severity。
- Impact。
- Fixed Version。
- Mitigation。
- Credit。
- Timeline。

## 26. 利用细节

公开 PoC/利用细节的时间和深度应根据修复可用性、客户升级情况和实际风险决定。

## 27. Customer Notification

重大漏洞：

- 主动通知高风险客户。
- 给出明确版本和操作步骤。
- 提供 Workaround。
- 提供升级验证方式。

## 28. Cloud/SaaS

SaaS 漏洞如果公司已服务端修复，也应根据事件影响判断是否需要客户通知或安全公告。

## 29. Secret/数据报告

外部报告如果包含客户数据或 Secret：

- 立即限制证据传播。
- 吊销 Secret。
- 评估数据事件。
- 请求 Reporter 最小化保存。

## 30. Report Retention

PSIRT Case 保存：

- 原始报告。
- 沟通。
- 技术验证。
- Fix。
- Release。
- Advisory。
- Disclosure。

## 31. Root Cause

重大产品漏洞修复后分析：

- 为什么产生。
- 为什么测试没发现。
- 是否影响其他产品。
- 是否应增加 SDL 控制。

## 32. Systemic Fix

例如：

- 统一 Auth SDK。
- 禁用默认密码。
- CI Rule。
- SAST Rule。
- Security Test。
- Reference Architecture。

## 33. SLA

建议公司定义：

- Acknowledge。
- Triage。
- Critical Fix Target。
- Researcher Update Frequency。

避免收到报告数周无反馈。

## 34. Metrics

- 报告数量。
- 有效率。
- 首次响应时间。
- Triage 时间。
- Critical 修复时间。
- Duplicate 比例。
- Reporter 满意度（可选）。
- 重复漏洞类型。

## 35. P0

1. security@company。
2. PSIRT Owner。
3. Case ID。
4. 接收/验证/修复流程。
5. Advisory 模板。
6. 客户通知流程。

## 36. P1

1. VDP。
2. security.txt。
3. CVE 流程。
4. Researcher Communication Guide。
5. Support/EOL 联动。

## 37. P2

1. Bug Bounty。
2. PSIRT Platform。
3. 自动受影响版本识别。
4. SBOM 与客户影响分析。

## 38. 审计证据

- Case。
- Reporter Communication。
- Validation。
- Risk。
- Fix。
- Release。
- Advisory。
- Customer Notification。
- Root Cause。

## 39. 当前待确认

- [ ] 安全报告邮箱
- [ ] PSIRT Lead
- [ ] 法务联系人
- [ ] Advisory 发布渠道
- [ ] CVE 策略
- [ ] VDP/奖励计划
- [ ] 外部响应 SLA

## 40. 参考

- CISA Vulnerability Disclosure Program 思路
- NIST SSDF
- 公司 Product Security 与漏洞管理规范

## 41. 变更记录

| 版本 | 日期 | 说明 |
|---|---|---|
| V0.1 | 2026-09 | 建立 PSIRT、VDP 与协调披露流程 |

<!-- obsidian-relations:start -->
## Obsidian 关联知识

> [!tip] 图谱导航
> 本区由 scripts/obsidian_optimize.py 维护，用于 Obsidian Global Graph / Local Graph。业务正文请维护在上方章节。

- **上级导航**：[[docs/公司安全知识图谱|公司安全知识图谱]]
- **前置知识**：[[docs/09-漏洞管理与授权渗透测试|09 漏洞管理与授权渗透测试]] · [[docs/18-产品安全与默认安全|18 Product Security 与 Secure by Default]]
- **下游知识**：[[docs/11-安全事件与应急响应|11 安全事件与应急响应]]
- **横向关联**：[[docs/27-第三方风险管理|27 第三方风险管理]]

<!-- obsidian-relations:end -->
