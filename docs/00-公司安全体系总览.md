...
- [Control Catalog](governance/安全控制目录.md)：统一 Requirement 与 Control ID。
- [Current / Target Profile](governance/安全现状与目标画像.md)：只依据真实证据评估当前能力。
- [Security RACI](governance/安全职责RACI.md)：明确 R/A/C/I。
- [Company Security Parameters Register](governance/公司安全参数登记表.md)：统一维护平台、版本、SLA、Owner、RTO/RPO 等公司参数。
- [Document Hierarchy](governance/文档层级与维护规则.md)：统一 Policy、Standard、SOP、Playbook、Baseline 和 Template。
...
各章节中的待确认项由 [公司安全参数登记表](governance/公司安全参数登记表.md) 统一管理。Domain 文档可保留必要上下文，但参数登记表是 V2.0 的统一 Source of Truth。
